...
def generate_launch_description():
    ...
    slam_params_file_arg = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            get_package_share_directory("basketbuddy"), "launch", "slam.yaml"
        ),
    ...
    lidar_launch = IncludeLaunchDescription(
            PythonLaunchDescriptionSource([
               os.path.join(get_package_share_directory("ldlidar_node"), "launch"), "/ldlidar.launch.py"])
        )
    ...
    ld.add_action(Node(
        ...
        arguments="0 0 0 0 0 0 ldlidar_link odom".split(),
    ))
    ld.add_action(Node(
        package="tf2_ros",
        executable="static_transform_publisher",
