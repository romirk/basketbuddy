...
void initialize()
{
	...
	SignalMessage::send(S_Startup);
	// blocking_wait_for(S_Startup);
	LogMessage::send("BasketBuddy Core");
	LogMessage::send(VERSION_DATE);
...
String padString(String str, unsigned len)
{
...
int blocking_wait_for(SignalType signal, int timeout = 1000)
{
	char buf[CMD_BUFFER_SIZE];
	auto ack = (new SignalMessage(signal))->serialize();
	while (Serial.available() && (timeout == -1 || timeout-- > 0))
	{
		Serial.readBytes(buf, CMD_BUFFER_SIZE);
		if (strncmp(buf, ack, CMD_BUFFER_SIZE) == 0)
		{
			return 0;
		}
		delay(1);
